...
%% load workspaces
load([InputDir '\Steadystate_workspace_paper.mat']);
...
%     tab(row,col) = sum(bmk.Rb(tset).*tstep(tset))./ sum(tstep(tset))-Rb ;
    ...
    row = row+1;
    if AdjGovBudgetConstraint == 1
        x = sum(dtot.Y(tset).*tstep(tset)./Y)./ sum(tstep(tset));
        tab(row,col)=mean(x);
    else
    ...
    row = row+1;
    if AdjGovBudgetConstraint == 1
%         x = sum(dtot.G(tset).*tstep(tset)./G)./ sum(tstep(tset));
        x = sum(dtot.G.*tstep(tset)./G)./ sum(tstep(tset));
        tab(row,col)=mean(x);
    else
...
%writematrix(finaltable,'Table7_and_8_col_1.txt');

writematrix(finaltable,[TransTableOutDir transtable_fname]);
