clc;
%clearvars -except count MonetaryShockSize;
% close all;
fprintf('Plotting Figures 5 and 6\n');

if MonetaryShockSize == 0.0025
   figname = 'NegativeMPS.png';
else
   figname = 'PositiveMPS.png'; 
end    


%% PARAMETERS

% BaseInputDir = strcat('D:\University of Washington\HANK Literature\Codes for Heterogenous Models',...
%     '\myHANKReplication\ReplicateHank-Steady State\'); 
BaseInputDir = strcat(cd,'\');
SaveFigsDir = strcat(BaseInputDir,'TransitionFigures'); 
...
%%
load([BaseInputDir 'Steadystate_workspace_paper.mat']);
...
%8 is all at once
fprintf('Consumption Responses by Liquid Wealth Position\n');
count=count+1;
figure(count);
...
if Save==1
    saveas(gcf,[SaveFigsDir '\fig_5a.png' figname locfigname]);
...
if Save==1
    saveas(gcf,[SaveFigsDir '\fig_5b.png' figname locfigname]);
...
set(leg,'FontSize',16);
if Save==1
    saveas(gcf,[SaveFigsDir '\fig_6a_' figname locfigname]);
...
set(leg,'FontSize',16);
if Save==1
    saveas(gcf,[SaveFigsDir '\fig_6b_' figname locfigname]);
